# ============================================================
# Dent Blanche - Business Statistics 2 - Final Project
# Obesity status of Latin American women
# ============================================================

# creation our database - We import a database from Kaggle but only with women filtered with Excel
setwd("C:/Users/MSI/Documents/3eme semestre bachelor/business statistics 2/projet")
getwd()
women <- read.csv("DataBaseWomen.csv", header = TRUE, sep = ";")

# selection of variables from the original database
women2 <- women[ , c("Age", "Weight","Height","FAF","IMC")]
...
df <- df[order(-df$Percentage), ] # Now shown in decreasing order
df
...
# The function
qqnorm <- function(x, main = "QQ-Plot",
                   xlab = "Quantiles for the normal distribution",
...
prop_for_diff_sd <- function(data, x) {
  length(data[data <= mean(data) + x * sd(data) &
              data >= mean(data) - x * sd(data)]) / length(data)
}
...
qqnorm(x, main = "QQ plot with Normal Distribution")

# -------------------------------------------------------------------
# Code for analysis of our variables
# -------------------------------------------------------------------
...
estim_mnop1  <- mean(nop_age, na.rm = TRUE)  # estimation of the mean age of non-obese women
estim_mnop1

op_age       <- women2$age[women2$obesity_level == 1]
estim_op1    <- mean(op_age, na.rm = TRUE)   # estimation of the mean age of obese women
estim_op1
...
estim_mnop2 <- mean(nop_pal, na.rm = TRUE)  # estimation of the mean pal of non-obese women
estim_mnop2

op_pal      <- women2$pal[women2$obesity_level == 1]
estim_op2   <- mean(op_pal, na.rm = TRUE)   # estimation of the mean pal of obese women
estim_op2
...
estim_wp_imc_level <- mean(wp_imc_level, na.rm = TRUE)  # estimation of the mean imc_level
estim_wp_imc_level
